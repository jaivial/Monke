/* MONKE-DNA cross-platform inference runtime (streaming server over stdin/stdout).
 * DNA-DiskChat-2B: ternary-free fp controller in RAM, CODON memory table on disk,
 * 2 rows/token (128 B each) read unbuffered (O_DIRECT / F_NOCACHE / unbuffered
 * ReadFile) and DECODED through a positional codebook filter, then expand/contract.
 *
 * Protocol (identical to monke_runtime.c so the Electron engine can drive it):
 *   RESET                     -> clear recurrent state (new chat)
 *   PROMPT <id> <id> ...      -> feed tokens into state (no generation) -> "OK <last>"
 *   GEN <max> <startTok>      -> greedy-generate; emits "T <id>" per token, then
 *                                "E <tok_s> <io_bytes_per_token> <rss_mb>"
 *   PING                      -> "PONG"
 * Args: controller.bin codons.u8 [scale-ignored] [threads]
 */
#define _GNU_SOURCE
#include <math.h>
#include <stdint.h>
...
static void die(const char*s){fprintf(stderr,"[monke-dna] fatal: %s\n",s);exit(1);}

int V,D,L,FF,A,B,NCOD;
typedef struct{float*n1w,*n1b,*proj,*o,*n2w,*n2b,*up,*down,*decay;}Blk;
float *embed,*normw,*normb,*ra,*rb,*ca,*cb,*expandw,*contractw,*codebook; Blk*blk;
...
/* ---- unbuffered disk read of one page holding a codon row + codebook DECODE ---- */
uint64_t io_total=0;
#if defined(_WIN32)
static HANDLE hfile;
...
static void readrow_raw(uint64_t key,void*pg,uint8_t**cod){
  long long off=(long long)key*NCOD, base=off & ~4095LL;
  ...
  io_total+=4096; *cod=(uint8_t*)pg+(off-base);
}
...
static void open_table(const char*p){
  ...
  if(mfd>=0) fcntl(mfd,F_NOCACHE,1);
  #endif
 #endif
  if(mfd<0) die("open table");
}
static void readrow_raw(uint64_t key,void*pg,uint8_t**cod){
  off_t off=(off_t)key*NCOD, base=off & ~4095L;
...
#endif
static void readdecode(uint64_t key,void*pg,float*out){
  uint8_t*c; readrow_raw(key,pg,&c);           /* NCOD codon bytes (fits in page) */
  for(int ci=0;ci<NCOD;ci++){int fr=ci&3;const float*e=codebook+(((size_t)fr*256+c[ci])*4);
    out[ci*4+0]=e[0];out[ci*4+1]=e[1];out[ci*4+2]=e[2];out[ci*4+3]=e[3];}
}

/* ---- persistent state + scratch ---- */
float*state; float *x,*h,*q4,*tmp,*xo,*q2,*fin,*xd,*xa,*xb,*r0,*r1,*ext,*e2,*ex,*feat,*logits;
void*pg0,*pg1;
static void alloc_bufs(){
  ...
  xa=malloc(A*4);xb=malloc(B*4);r0=malloc(D*4);r1=malloc(D*4);ext=malloc(D*4);e2=malloc(2*D*4);ex=malloc(D*4);feat=malloc(D*4);logits=malloc((size_t)V*4);
  #if defined(_WIN32)
...
static int step(int tok){
  ...
  float e=expf(s1-s0),al1=e/(1+e),al0=1-al1;readdecode(k0,pg0,r0);readdecode(k1,pg1,r1);
  for(int i=0;i<D;i++)ext[i]=al0*r0[i]+al1*r1[i];
  mv(expandw,ext,e2,2*D,D);for(int i=0;i<2*D;i++)e2[i]=silu(e2[i]);mv(contractw,e2,ex,D,2*D);
  for(int i=0;i<D;i++)tmp[i]=x[i]+ex[i]+ca[(size_t)a0*D+i]+cb[(size_t)b0*D+i];
  lnorm(tmp,normw,normb,feat);
  mv(embed,feat,logits,V,D);int best=0;for(int i=1;i<V;i++)if(logits[i]>logits[best])best=i;return best;
}

int main(int ac,char**av){
  if(ac<3){fprintf(stderr,"usage: monke_runtime_dna controller.bin codons.u8 [scale-ignored] [threads]\n");return 2;}
  #ifdef _OPENMP
  if(ac>4) omp_set_num_threads(atoi(av[4]));
  #endif
  FILE*f=fopen(av[1],"rb");if(!f)die("open controller");char mg[4];int hd[6];
  if(fread(mg,1,4,f)!=4||fread(hd,4,6,f)!=6||memcmp(mg,"DNA1",4))die("bad controller header");
  V=hd[0];D=hd[1];L=hd[2];FF=hd[3];A=hd[4];B=hd[5];NCOD=D/4;
  embed=rd(f,(long)V*D); blk=calloc(L,sizeof(Blk));
  for(int l=0;l<L;l++){Blk*z=&blk[l];z->n1w=rd(f,D);z->n1b=rd(f,D);z->proj=rd(f,(long)4*D*D);z->o=rd(f,(long)D*D);
    z->n2w=rd(f,D);z->n2b=rd(f,D);z->up=rd(f,(long)2*FF*D);z->down=rd(f,(long)D*FF);z->decay=rd(f,D);}
  normw=rd(f,D);normb=rd(f,D);ra=rd(f,(long)A*D);rb=rd(f,(long)B*D);ca=rd(f,(long)A*D);cb=rd(f,(long)B*D);
  expandw=rd(f,(long)2*D*D);contractw=rd(f,(long)D*2*D);codebook=rd(f,4*256*4);fclose(f);
  open_table(av[2]); alloc_bufs();
